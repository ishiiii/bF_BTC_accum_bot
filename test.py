...
print(ticker_bf["last"])


#print(str(ticker_bf["last"]) + str(ticker_liquid["last"]) + str(ticker_zaif["last"]) + str(ticker_cc["last"]))
...
#Liquidが安い場合の処理
    try:
        ...
        msg = 'BTCの現在価格は' + str(ticker_liquid["last"]) + 'です。\n' + 'エラーが発生したため、BTCを購入できませんでした。' #エラーが返ってきた場合購入できていないのでこの文言を送信
    else:
        msg = 'BTCの現在価格は' + str(ticker_liquid["last"]) + 'です。\n' + str(ticker['last'] / 1000) + '円で0.001BTCを購入しました'#エラー以外ならば購入しているのでこの文言を送信
...
#zaifが安場合の処理
    try:
        ...
        msg = 'BTCの現在価格は' + str(ticker_zaif["last"]) + 'です。\n' + 'エラーが発生したため、BTCを購入できませんでした。' #エラーが返ってきた場合購入できていないのでこの文言を送信
    else:
        msg = 'BTCの現在価格は' + str(ticker_zaif["last"]) + 'です。\n' + str(ticker['last'] / 1000) + '円で0.001BTCを購入しました'#エラー以外ならば購入しているのでこの文言を送信
...
#ccが安い場合の処理
    try:
        ...
        msg = 'BTCの現在価格は' + str(ticker_cc["last"]) + 'です。\n' + 'エラーが発生したため、BTCを購入できませんでした。' #エラーが返ってきた場合購入できていないのでこの文言を送信
    else:
        msg = 'BTCの現在価格は' + str(ticker_cc["last"]) + 'です。\n' + str(ticker['last'] / 1000) + '円で0.001BTCを購入しました'#エラー以外ならば購入しているのでこの文言を送信
    line_token = 'XXXXXXXXXXX'#個人で取得したLineNotify用のトークン
    line_api = 'https://notify-api.line.me/api/notify'
